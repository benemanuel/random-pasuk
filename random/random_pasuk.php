<?php
	...
	$home_url="https://geulah.org.il/Tanach/Tanach.xml";
...
//	echo "<div style='visibility: hidden'>The number is: $random</div><br>";
	if($random > 23212) {
	  die("Random seed wrong" . $random);
	}
        if ($random < 0)  {
	    	 $random = ($randval%23212);
		 $pol = date('d-m-Y H:i:s');
//		 echo "random generated " . $random . " at " . $pol;
	} 
	$aline='bbbcccvvv';
	$v=$random*13; #"bbb ccc vvvCL"
	# beginning of verse
	if(!file_exists("bible.db")) {
	  die("bible database file missing");
	  } else {
